...
class Sparkline(object):
    ...
    def add(self, ts, key, count, color=''):
        if ts not in self.data:
            self.recalc_active(ts)
        self.data[ts][key] = (color, count)

    def get_sparkline(self, key, baseline='zero', spark_len=None):
        if spark_len <= 0:
            return ''
        if spark_len is None:
        ...
        if active_data:
            ...
            if baseline == 'zero':
                min_val = 0
                step = float(max(values)) / (len(self.ticks) - 1)
            elif baseline == 'min':
                min_val = min(values)
                step = float(max(values) - min_val) / (len(self.ticks) - 1)
            result = []
            for color, value in active_data:
                if value == 0:
                    tick = ' '
                else:
                    rank = int((value - min_val) / step)
...
class Screen(object):
    ...
    def __init__(self, info_panel_width, markup_provider, **kwargs):
        ...
        times_block = VerticalBlock(AnswSizesBlock(self), AvgTimesBlock(self, **times_args), self)
...
class PercentilesBlock(AbstractBlock):
    ...
    def add_second(self, data):
        ...
        if len(self.last_min) >= 60:
            self.last_min.pop(0)
        self.last_min.append(dist)
    ...
    def render(self, expected_width=None):
        ...
        if self.overall is None:
            ...
            last_times = hist_to_quant(self.last_min[-1], self.quantiles)
            ...
            last_1m = self.last_min[0]
            for s in self.last_min[1:]:
                last_1m = last_1m.add(s, fill_value=0)
...
class AnswSizesBlock(AbstractBlock):
    ...
    def __init__(self, screen):
        ...
        self.title = 'Request and Response Sizes (all/last):'
        template = {
            'name': {'tpl': '{:>}'},
            'avg': {'tpl': '{:>,.1f} b'},
            'last_avg': {'tpl': '{:>,.1f} b'}
    ...
    def render(self, expected_width=None):
        ...
        if self.overall['count']:
            ...
            prepared += self.formatter.render_table(data, ['name', 'avg', 'last_avg'])
        else:
            self.lines.append(('',))
            self.lines.append(('',))
        self.width, self.lines = self.fill_rectangle(prepared)
...
class AvgTimesBlock(AbstractBlock):
    ...
    def __init__(self, screen, times_max_spark=60):
        AbstractBlock.__init__(self, screen)
